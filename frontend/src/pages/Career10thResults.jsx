import "../styles/Career10thResults.css";

export default function Career10thResults() {
  const rawResults = JSON.parse(localStorage.getItem("career10thResults")) || [];
  ...
  if (!Array.isArray(rawResults) || rawResults.length === 0) {
    return (
      <div className="results-page">
        <h2 className="no-results">No recommendations found. Please try again.</h2>
      </div>
    );
  }
              ...
              <div className="college-list">
                {(item.colleges || []).slice(0, 4).map((c, i) => (
                  <span key={i} className="college-chip">
                    {c.college_name || c.name || c.college || "Unknown College"}
                  </span>
                ))}
              </div>
            </div>
